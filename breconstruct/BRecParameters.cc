...
int BRecParameters::GetImpulseNumber(int i) const {
  assert(i >= 0 && i < fNhit);
  return fImpulseNumbers[i];
}

//______________________________________________________________________________
void BRecParameters::SetImpulseNumbers(int *ns) {
  assert(fNhit > 0);
  if (fImpulseNumbers) delete [] fImpulseNumbers;
  fImpulseNumbers = new int[fNhit];
  for (int i = 0; i < fNhit; i++)
    fImpulseNumbers[i] = ns[i];
}
