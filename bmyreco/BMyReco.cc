...
BMyReco::BMyReco(string fname)
{
  ...
  hChargeNoLED=new TH1F("hChargeNoLED","hChargeNoLED",3000,0,30000);
  hMapOfHits100=new TH2F("hMapOfHits100","hMapOfHits100",21,-0.5,20.5,25,-0.5,24.5);
  hMapOfHits600=new TH2F("hMapOfHits600","hMapOfHits600",21,-0.5,20.5,25,-0.5,24.5);
...
Int_t BMyReco::Process()
{
  ...
  if (iEvent%100000==0) std::cout<<"eventNumber: "<<iEvent<<std::endl;
...
std::vector<int> BMyReco::filterHits(float minAmpl)
{
  ...
  for (int j=0; j< impulse_n; j++){
    ...
    if (nch!=118&&nch!=119) hChargeNoLED->Fill(fExtractedImpulse->GetQ(j),1);
    //    std::cout<<"nch: "<<nch<<std::endl;
    if (fExtractedImpulse->GetQ(j)>100) hMapOfHits100->Fill(floor((nch)/24),(nch)%24+1);
    if (fExtractedImpulse->GetQ(j)>600) hMapOfHits600->Fill(floor((nch)/24),(nch)%24+1);
