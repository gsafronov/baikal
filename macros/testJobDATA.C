int testJobDATA()
{
  ...
  MReadTree  reader("Events", "data_new.root");
  ...
  //telreader.SetSource("/data/processed/e0556.root", "Configuration"); // file with MainInfo
  telreader.SetSource("OUT/2015/e0556.root", "Configuration"); // file with MainInfo
  tasks.AddToList(&telreader);
  
  // dynamic geometry 
  BEvoGeomApplyLinear geom;
  geom.SetGeometryData("e0556.evo.beacons.aligned.guessed.root");
  geom.SetStringTemplate("../config/stringtemplate_2sec");
  //geom.SetVerbose(kTRUE);
  tasks.AddToList(&geom);

  BMyRecoBEvt myrecoTest("foutDATA.root", false);
