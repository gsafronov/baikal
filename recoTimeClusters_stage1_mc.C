...
void recoTimeClusters_stage1_mc(string mc_infile="/scratch/safronov/baikal/mc/cors-n8x10m-2sec-tr112-thr4.0-1.5.wout", string root_infile="OUT/outout.root",  int calibChanID=10, Bool_t isrec = kTRUE)
{
    ...
    */
    ...
    BMyRecoMC mcAnalysis("mcAnalysis.root");
    tasks.AddToList(&mcAnalysis);
    ...
    */
    ...
    BTimeClusters clusterise;
    clusterise.SetInputMaskName("AmplitudeFilterMask");
    clusterise.SetOutputMaskName("TimeClusterFilterMask");
    tasks.AddToList(&clusterise);

    BFQualify qual;
    qual.SetFilterEventMaskName("TimeClusterFilterMask");
    tasks.AddToList(&qual);
    
    BReconstructMuon muonrec;
    muonrec.SetEventMaskName("TimeClusterFilterMask");
    muonrec.SetInitialsType(0);
    muonrec.SetVerbose(kTRUE);

    std::cout<<"SCRIPT:  qualify"<<std::endl;
    
    BRecQualify recqual;
    recqual.SetHitCriterionFlag();
    recqual.SetQ10Path(gSystem->ExpandPathName("/scratch/safronov/baikal/bars3/trunk/config/quant10HE.data"));
    recqual.SetFilterEventMaskName("TimeClusterFilterMask");
